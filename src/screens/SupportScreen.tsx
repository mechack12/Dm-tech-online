...
// ── Gemini client ────────────────────────────────────────────────────────────
const ai = new GoogleGenAI({ apiKey: 'AIzaSyARJhGifRa-alFp-QtyJEBj15YsYW1YIR0' });
...
export function SupportScreen() {
  ...
  const sendToGemini = async (fullPrompt: string) => {
    ...
    if (lastErr) {
      ...
      const friendly =
        raw.includes('API key') || raw.includes('PERMISSION_DENIED')
          ? 'Authentication failed — please check your Gemini API key in the .env file.'
          : raw.includes('quota') || raw.includes('RESOURCE_EXHAUSTED')
          ? 'Your Gemini API key has exceeded its quota. Get a free key at aistudio.google.com and update the .env file.'
          : raw.includes('NOT_FOUND') || raw.includes('not found')
          ? 'No AI models are available for this API key. Please check your key at aistudio.google.com.'
